...
ToggleWallHack()
{
    if(self.ToggleWallHack==true)
    {
        ...
        self maps\mp\killstreaks\_spyplane::callsatellite("radardirection_mp");
        self iprintln("ESP Box ^2[ON]");
        self.ToggleWallHack=false;
    }
    else
    {
        self thread disableESP();
        self iprintln("ESP Box ^1[OFF]");
...
Sclan1()
{
	setdvar( "clanName", "xRAx" );
	self iprintln( "^2Clan tag set to ^2xRAx" );

}

Sclan2()
{
	setdvar( "clanName", "FAG" );
	self iprintln( "^2Clan tag set to ^2FAG" );

}

Sclan3()
{
	setdvar( "clanName", "FUCK" );
	self iprintln( "^2Clan tag set to ^2Fuck" );

}

Sclan4()
{
	setdvar( "clanName", "3arc" );
	self iprintln( "^2Clan tag set to ^23arc" );
...
{
	setdvar( "clanName", "^2" );
	self iprintln( "^2Clan tag set to ^2Color green" );

}

Sclan6()
{
	setdvar( "clanName", "^1DEV" );
	self iprintln( "^2Clan tag set to ^1Dev" );
