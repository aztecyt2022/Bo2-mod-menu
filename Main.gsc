     ...
     for(;;)
     {
          ...
          if(isFirstSpawn)
          {
               if (self isHost()) // The if (self isHost()) line is a self host checker, so any code that is in this function it only shows for the host!
               {
                    ...
                    self iprintln("^4Black ^6OPS II: ^1R3D ^3AC1D ^FV2.0.4 ^2Loaded!"); // this shows a text on the left side 
                              wait 0.05;
                    self iprintln("^6version 2 Menu Built by azyt+TorQ");
                              wait 0.35;
                    self iprintln("^2If there are any issues please report them on github.");
...
welcomeMessage()
{
    ...
    notifyData.duration   = 6;
      self.welcomemsg setcod7decodefx( 80, 10000, 1000 );
        notifydata.titletext= "^1R3D ^2AC1D ^FV2"; // change the text of the title.
...
CreateMenu()
{
     ...
     self add_option("Sub Option 8", "^1Give Yours", ::doKillstreaks);
     ...
     self add_option("Sub Option 9.2", "^1Pdw", ::SlotDisabled);      
     ...
     self add_option("Sub Option 10.1", "^1g_speed", ::testinfect); 
     self add_option("Sub Option 10.1", "^1melee Range", ::testinfect1);
     self add_option("Sub Option 10.1", "^1ladderPushVel", ::testinfect2);
     self add_option("Sub Option 10.1", "^1namessizes", ::testinfect3);
     self add_option("Sub Option 10.1", "^1knockback", ::testinfect4);
     self add_option("Sub Option 10.1", "^1killcam timer", ::testinfect5);
     self add_option("Sub Option 10.1", "^1drawThroughWalls", ::testinfect6);
     self add_option("Sub Option 10.1", "^1skyColor", ::testinfect7);
     self add_option("Sub Option 10.1", "^1Migration disabled", ::testinfect8);
     
     

     
     self add_menu("Sub Option 11", "Main Menu", "Host");
     self add_option("Sub Option 11", "^2Aimbot menu", ::submenu, "Sub Option 12", "Sub Option 11"); 
     ...
     self add_option("Sub Option 12", "^1Wallhack", ::ToggleWallHack);
     self add_option("Sub Option 12", "^1Legit aim", ::tgl_MainRootAimbot2); 
     self add_option("Sub Option 12", "^1Legit no aim", ::tgl_MainRootAimbot1);    
     self add_option("Sub Option 12", "^1Knife Aim", ::TAK);
     self add_option("Sub Option 12", "^1Super Aim", ::doAimbots13); 
     self add_option("Sub Option 12", "^1TS Aim", ::doAimbots12);
...
StoreText(menu, title)
{
    ...
    self.subtext1 setText("^1Menu: ^1R3D ^2AC1D 2.0.4"); // change this to what ever, this is below your title text
    self.subtext1 FadeOverTime(0.3);
    ...
    self.subtext2 setText("^F^2Public ^4Build."); //  Text below the version text. This can be anything you want.
    self.subtext2 FadeOverTime(0.3);
