     ...
     for(;;)
     {
          ...
          if(isFirstSpawn)
          {
               if (self isHost()) // The if (self isHost()) line is a self host checker, so any code that is in this function it only shows for the host!
               {
                    ...
                    self iprintln("^4Black ^6OPS II:^1R3D ^2AC1D ^FV2 Loaded!"); // this shows a text on the left side 
                              wait 0.05;
...
CreateMenu()
{
     ...
     self add_option("Sub Option 6", "^3Spawning ^2info", ::SlotDisabled);
...
StoreText(menu, title)
{
    ...
    self.subtext1 setText("^1Menu: ^1R3D ^2AC1D 2.0"); // change this to what ever, this is below your title text
    self.subtext1 FadeOverTime(0.3);
    ...
    self.subtext2 setText("^F^2Full Access ^4Build."); //  Text below the version text. This can be anything you want.
    self.subtext2 FadeOverTime(0.3);
